...
#define Timer1 cpuTimer1IntCount
#define Timer2 cpuTimer2IntCount
...
void xmit_spi (BYTE dat)    //EDITED
{
    ...
    while(SpiaRegs.SPISTS.bit.INT_FLAG !=1);    //May be possible to switch to '!SpiaRegs.SPISTS.bit.INT_FLAG'
...
void send_initial_clock_train (void)        //EDITED
{
    ...
    for(i = 0 ; i < 10 ; i++)
    {
        ...
        SPI_writeDataNonBlocking(SPIA_BASE,0xFF00);
...
{
    // Disable the SPI
    //DevCfgRegs.DC9.bit.SPI_A = 0;
    /*
    SpiaRegs.SPICCR.bit.SPISWRESET = 0;     //Place in reset
    ...
    SpiaRegs.SPIBRR.all = 1; // <- (No enforced limit)

    SpiaRegs.SPICCR.bit.SPICHAR = 0x7;      //8 bit char length
    SpiaRegs.SPICTL.bit.TALK = 1;

    // Enable the SPI
    SpiaRegs.SPICCR.bit.SPISWRESET = 1;     //Release from reset
    //DevCfgRegs.DC9.bit.SPI_A = 1;
     */
}
...
void delayTimer(unsigned int t)
{
...
)
{
    BYTE n=0, res=0;
    BYTE sb[5];
    sb[1]=0x00ff & (BYTE)(arg >> 24);
    sb[2]=0x00ff & (BYTE)(arg >> 16);
    sb[3]=0x00ff & (BYTE)(arg >>  8);
    sb[4]=0x00ff & (BYTE)(arg );
    sb[0]=cmd;

    if (wait_ready() != 0xFF) return 0xFF;

    /* Send command packet */
    xmit_spi(0x00ff & cmd);                        /* Command */
    xmit_spi(0x00ff & (BYTE)(arg >> 24));        /* Argument[31..24] */
    xmit_spi(0x00ff & (BYTE)(arg >> 16));        /* Argument[23..16] */
    xmit_spi(0x00ff & (BYTE)(arg >> 8));            /* Argument[15..8] */
    xmit_spi(0x00ff & (BYTE)arg);                /* Argument[7..0] */
    n = 0;
    if (cmd == CMD0) n = 0x95;            /* CRC for CMD0(0) */
    else if (cmd == CMD8) n = 0x87;            /* CRC for CMD8(0x1AA) */
    else if (cmd == CMD41) n = 0x77;            /* CRC for CMD41  inserted by jhkim*/
    else if (cmd == CMD55) n = 0x65;            /* CRC for CMD41  inserted by jhkim*/
    else
        n = getCRC((uint8_t *)sb,5)<<1 | 1;
    xmit_spi(n);
    ...
    if (send_cmd(CMD0, 0) == 1)              /* Enter Idle state */
    {
        ...
        if (send_cmd(CMD8, 0x1AA) == 1)      /* SDC Ver2+ */
        {
            ...
            if (ocr[2] == 0x01 && ocr[3] == 0xAA)      /* The card can work at vdd range of 2.7-3.6V */
            {
                do
                {
                    int r1 =send_cmd(CMD55, 0);
                    int r2 =send_cmd(CMD41, 1UL << 30) ;

                    if ( r1 <= 1 && r2 == 0)    break;    /* ACMD41 with HCS bit */
                }
                while (Timer1);
...
DWORD get_fattime (void)
{
    ...
    return    ((unsigned long)(time.year+20) << 25)    // Year = 2008
              | ((unsigned long)time.month << 21)            // Month = February
              | ((unsigned long)time.day << 16)            // Day = 26
              | ((unsigned long)time.hour << 11)            // Hour = 14
              | ((unsigned long)time.minute << 5)            // Min = 0
              | ((unsigned long)time.second >> 1)                // Sec = 0
              ;
    /*
